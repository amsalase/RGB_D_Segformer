...
class Engine(object):
    def __init__(self, custom_parser=None):
        ...
        if self.distributed:
            ...
            logger.info(f"Initializing process group with backend 'nccl'")
            
            dist.init_process_group(backend="nccl", world_size=self.world_size, init_method='env://', timeout=timedelta(seconds=300))
